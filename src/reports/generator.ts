...
function analyticsSection(analytics: Analytics, emptyMessage: string): string {
  if (analytics.total_records === 0) {
    return `<p style="color:#71717a;font-size:13px;">${emptyMessage}</p>`;
  }
...
export function renderReport(
  ...
  const formattedDate = options.reportDateLabel ?? new Date(reportDate + "T12:00:00Z").toLocaleDateString("en-US", {
    month: "long", day: "2-digit", year: "numeric", timeZone: "UTC",
  });
  const title = options.title ?? "Daily Patient Report";
  const analyticsHeading = options.analyticsHeading ?? "Voice AI Daily Analytics";
  const analyticsSubheading = options.analyticsSubheading ?? "Previous Day";
  const analyticsEmptyMessage =
    options.analyticsEmptyMessage ?? "No appointment data available for the previous day.";
  const windowDescription =
    options.windowDescription ??
    "This is an automated report generated by PatientXpress AI Voice, covering activity between <strong>12:00 AM and 11:59 PM</strong>.";
...
<hr style="border:none;border-top:1px solid #d4d4d8;margin:24px 0;">
<p style="margin:0 0 4px;"><strong>${analyticsHeading}</strong> <span style="font-size:13px;color:#71717a;">— ${analyticsSubheading}</span></p>
${analyticsSection(analytics, analyticsEmptyMessage)}
<hr style="border:none;border-top:1px solid #d4d4d8;margin:24px 0;">
<p style="color:#52525b;">${windowDescription}<br><br>If you have any questions or would like additional insights, please let us know.</p>
